...
export async function handlerVideoGet(cfg: ApiConfig, req: BunRequest) {
  ...
  const signedVideo = video.videoURL ? dbVideoToSignedVideo(cfg, video) : video;
  console.log("get video", signedVideo);
...
export async function handlerVideosRetrieve(cfg: ApiConfig, req: Request) {
  ...
  const signedVideos = videos.map((video) => video.videoURL ? dbVideoToSignedVideo(cfg, video) : video);
